  ...
  "relative inline-flex shrink-0 cursor-pointer select-none items-center justify-center gap-2 whitespace-nowrap rounded-lg border border-border font-medium text-sm outline-none transition-shadow before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] pointer-coarse:after:absolute pointer-coarse:after:size-full pointer-coarse:after:min-h-11 pointer-coarse:after:min-w-11 hover:bg-bg-hover/50 focus-visible:ring-2 focus-visible:ring-border-emphasis focus-visible:ring-offset-1 focus-visible:ring-offset-white disabled:pointer-events-none disabled:opacity-64 data-[pressed]:bg-bg-hover data-[pressed]:text-fg data-[pressed]:transition-none data-[state=on]:bg-bg-hover data-[state=on]:text-fg data-[state=on]:transition-none [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    ...
    variants: {
      ...
      variant: {
        ...
        outline:
          "border-border bg-clip-padding shadow-xs not-disabled:not-active:not-data-[pressed]:not-data-[state=on]:before:shadow-[0_1px_--theme(--color-black/4%)] [&:is(:disabled,:active,[data-pressed],[data-state=on])]:shadow-none",
      },
    },
  },
);
